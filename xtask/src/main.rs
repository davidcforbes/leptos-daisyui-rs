...
fn tokens_css() -> String {
    use ui_tokens::{color, elevation, radius, spacing, stroke, typography as ty};
...
",
    ...
    // ldui-k4fn: the declared elevation vocabulary, as plain custom
    // properties on `:root` — deliberately OUTSIDE `@theme`.
    //
    // Inside `@theme` these would become Tailwind theme variables in an
    // unknown namespace, subject to Tailwind's own emission rules; here they
    // are ordinary CSS that survives the build verbatim, which is the whole
    // point of putting a contract in this file rather than in a component's
    // runtime `<style>`.
    //
    // **px, not rem** — the one place the DIP-to-rem rule above does not
    // apply. A shadow is a fixed depth cue, not a measure of text or space:
    // `ui_tokens::elevation` is the same DIP geometry the Direct2D face
    // draws as a Gaussian shadow, and the style audit compares computed
    ...
    for (name, shadow) in [
        ...
        ("4", elevation::LEVEL_4),
        ("8", elevation::LEVEL_8),
        ("16", elevation::LEVEL_16),
        ("64", elevation::LEVEL_64),
    ] {
        css.push_str(&format!(
            "  --ld-elevation-{}: {};\n",
            name,
    ...
    // paints one shadow and nothing else: no hover rule, no transition, no
    // transform, so it needs no reduced-motion or forced-colors handling of
    // its own. The two names share no lexical prefix beyond `ld-`, so
    // `grep ld-elevated` can never match this class or vice versa, and the
    // names describe different kinds of thing — `elevated` is a state a
    // surface can enter, `card-depth` is a fixed property a card has.
    //
    // LEVEL_4 is `ui_tokens::elevation`'s own documented "Card resting
    // elevation", and the level `.ld-elevated` also rests at — so an
    // interactive card and a static card sit at the same depth, which is
    // what makes a depth vocabulary mean anything. It is also the smallest
    // move from the stock `shadow-sm` this replaces (1px -> 2px offset,
    // 3px -> 4px blur, 0.10 -> 0.14 alpha). LEVEL_2 is the *control* outline
    // level; using it here would put cards shallower than every other card
    // surface in the system, re-creating the same drift facing the other way.
    //
    // `--ld-card-shadow` is the product-theme hook and is intentionally left
    // UNDEFINED by this crate, so the `var()` fallback is what paints by
    // default. A theme sets it once on `:root` (or any ancestor) and every
...
/// Format an elevation [`ui_tokens::elevation::Shadow`] as one CSS
/// `box-shadow` value.
///
...
/// from having to build the library it gates.
fn box_shadow(s: ui_tokens::elevation::Shadow) -> String {
    format!(
...
"#;
    ...
    #[test]
    fn reactivity_lane_has_exactly_69_browser_checks() {
        let source = include_str!("../../tests/reactivity_smoke.rs");
        assert_eq!(
            source.matches("#[tokio::test").count(),
            69,
            "the explicitly requested reactivity lane must keep its 69 checks \
             ...
             the fixed-slice fail-closed rejection; + 1 added for the \
             own-induced-refetch-across-differing-row-heights negative \
             ...
             header_actions; \n             + 1 added for ldui-j0mt's categorical LineChart secondary value \n             axis, which also raised that suite's own rootCount from 2 to 3 \n             because the charts demo gained a dual-axis chart; \n             + 1 added for ldui-kx7y's localizable DayScheduler event \n             accessible name, proving EN->ES->EN by same-DOM-node identity; \n             + 12 added for ldui-fqan's controlled Checkbox contract, including a \n             DECLINED click on a mixed box, which is what proves the component \n             re-asserts accepted truth rather than following the browser's \n             native toggle; \n             + 2 added for ldui-vshu's externally placed FilterSidebar toggle; \n             + 1 added for ldui-q73d's edge-action tooltip containment)"
        );
    }
...
mod gen_tokens_tests {
    ...
    fn generated_css_never_leaks_comment_text_outside_a_comment() {
        ...
        // `@layer components` / `:root` block braces, a
        // `--custom-property: value;` declaration, an `.ld-*` selector opener
        // (ldui-h7tw's type ramp, ldui-k4fn's `.ld-card-depth`), or a plain
        // `property: value;` declaration inside that selector.
        //
        ...
        fn assert_segment_is_only_declarations_or_structure(segment: &str, css: &str) {
            for line in segment.lines() {
                ...
                if trimmed.is_empty()
                    ...
                    // ldui-k4fn: the elevation custom properties live in a
                    // plain `:root` block outside `@theme`, so `var()` can
                    // resolve them with no dependency on Tailwind's theme
                    // emission rules.
                    || trimmed == ":root {"
                    || trimmed == "}"
                {
                ...
                // Any authored `.ld-*` rule opener: the six type-ramp steps
                // (ldui-h7tw) and `.ld-card-depth` (ldui-k4fn).
                let is_class_selector_opener =
                    trimmed.starts_with(".ld-") && trimmed.ends_with('{');
    ...
    // --- ldui-k4fn: static card elevation ---------------------------------

    #[test]
    fn elevation_custom_properties_derive_from_ui_tokens() {
        // Same shape as the table-colour test above: compute the expected
        // declaration from the live constant, so a sibling change to
        // `ui_tokens::elevation` is caught by the gate instead of quietly
        // leaving this file behind.
        let css = tokens_css();
        for (name, shadow) in [
            ("2", ui_tokens::elevation::LEVEL_2),
            ("4", ui_tokens::elevation::LEVEL_4),
            ("8", ui_tokens::elevation::LEVEL_8),
            ("16", ui_tokens::elevation::LEVEL_16),
            ("64", ui_tokens::elevation::LEVEL_64),
        ] {
            let want = format!("  --ld-elevation-{name}: {};\n", box_shadow(shadow));
            assert!(css.contains(&want), "missing {want:?} in:\n{css}");
        }
    }
    ...
    fn box_shadow_matches_the_runtime_preambles_format() {
        ...
        assert_eq!(
            box_shadow(ui_tokens::elevation::LEVEL_4),
            "0px 2px 4px rgba(0, 0, 0, 0.14)"
        );
    }

    #[test]
    fn card_depth_class_is_emitted_with_the_theme_override_hook() {
        let css = tokens_css();
        assert!(
            css.contains(
                "  .ld-card-depth {\n    box-shadow: var(--ld-card-shadow, var(--ld-elevation-4));\n  }\n"
    ...
    #[test]
    fn the_generated_stylesheet_defines_no_card_shadow_default() {
        // The hook only works because the framework never DECLARES
        // `--ld-card-shadow`: an undefined custom property is what makes the
        // `var()` fallback paint, and what lets a product theme set it on
        // `:root` with nothing to outrank. A default here would silently turn
        // the override into a specificity contest.
        let css = tokens_css();
        assert!(
            !css.contains("--ld-card-shadow:"),
            "--ld-card-shadow must stay undefined by the framework:\n{css}"
        );
    }

    #[test]
    fn the_static_card_elevation_carries_no_interaction_rules() {
        // `.ld-card-depth` is the NON-interactive counterpart to
        // `.ld-elevated`. If it ever grows a hover/transition/transform it
        // has become the class it exists not to be.
        let css = tokens_css();
        let rule_start = css.find(".ld-card-depth").expect("card depth rule present");
        let rule = &css[rule_start..];
        let rule = &rule[..rule.find('}').expect("rule closes")];
        for banned in ["hover", "transition", "transform"] {
            assert!(
                !rule.contains(banned),
                "the static card elevation must not carry `{banned}`: {rule}"
            );
        }
    }
}
